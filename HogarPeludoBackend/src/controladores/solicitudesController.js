...
const crearSolicitud = (req,res)=>{
     ...
     if(!req.params.id_mascota && !req.params.id_solicitante){
          res.status(500).json({type: 'error',mensaje: `El id de la solicitud y el id de solicitante no puede estar vacio`})
          return;
     }
...
const buscarSolicitudId = (req,res)=>{
     ...
     if(!id){
          res.status(500).json({type: 'error',mensaje: `El id de la solicitud no puede estar vacio`})
          return;
     }else{
